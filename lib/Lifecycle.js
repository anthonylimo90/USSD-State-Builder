...
class HealthCheck {
     ...
     * @param {Object} [options.startupChecks] - Startup checks to run once before marking ready
     * @param {number} [options.cacheTTL=0] - Cache health results for this many ms (0 = no cache)
     */
    constructor(options = {}) {
        this.stateMachine = options.stateMachine;
        this.storage = options.storage;
        this.customChecks = options.checks || {};
        this.startupChecks = options.startupChecks || {};
    ...
    /**
     * Run startup checks (should be called once during application boot)
     * @returns {Promise<Object>} Startup check result
     */
    async runStartupChecks() {
        const results = {};
        let allPassed = true;

        for (const [name, checkFn] of Object.entries(this.startupChecks)) {
            try {
                results[name] = await checkFn();
                if (!results[name].healthy) allPassed = false;
            } catch (error) {
        ...
        this._startupComplete = allPassed;
        this._startupErrors = results;
        return { ready: allPassed, checks: results };
    }

    /**
     * Liveness check - is the process alive and not deadlocked?
     * Lightweight check suitable for frequent polling.
     ...
     * Checks startup completion and all dependencies.
     * @returns {Promise<Object>} Readiness result
     */
    async readiness() {
        const startTime = Date.now();

        if (!this._startupComplete && Object.keys(this.startupChecks).length > 0) {
            return {
                status: 'not_ready',
                reason: 'startup_incomplete',
                timestamp: new Date().toISOString(),
                duration: Date.now() - startTime,
                startupErrors: this._startupErrors
            };
        }
    ...
    /**
     * Create an HTTP handler for liveness probes
    ...
    /**
     * Create an HTTP handler for readiness probes
     * @returns {Function} HTTP handler (req, res) => void
     */
    readinessHandler() {
        return async (req, res) => {
    ...
    async check() {
        ...
        if (this.cacheTTL > 0 && this._cachedResult) {
