...
const Validators = {
    ...
    /**
     * Validate input is a valid password with configurable strength requirements
    ...
    password: (options = {}) => async (input) => {
        ...
        if (input.length < minLength) {
            throw new ValidationError(message || `Password must be at least ${minLength} characters`);
        }

        if (requireUppercase && !/[A-Z]/.test(input)) {
            throw new ValidationError(message || 'Password must contain at least one uppercase letter');
        }

        if (requireLowercase && !/[a-z]/.test(input)) {
            throw new ValidationError(message || 'Password must contain at least one lowercase letter');
        }

        if (requireNumber && !/\d/.test(input)) {
            throw new ValidationError(message || 'Password must contain at least one number');
        }

        if (requireSpecial && !/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(input)) {
            throw new ValidationError(message || 'Password must contain at least one special character');
        }
    },

    /**
     * Validate input contains only alphanumeric characters (letters and numbers)
     * @param {string|Object} [options] - Error message or options object
     * @param {string} [options.message] - Custom error message
     * @returns {Function} Validator function
     */
    alphanumeric: (options = {}) => async (input) => {
        const message = typeof options === 'string' ? options : (options.message || 'Must contain only letters and numbers');
    ...
    /**
     * Validate input is a valid URL (http or https)
     * @param {string|Object} [options] - Error message or options object
     * @param {string} [options.message] - Custom error message
     * @returns {Function} Validator function
     */
    url: (options = {}) => async (input) => {
        const message = typeof options === 'string' ? options : (options.message || 'Invalid URL format');
    ...
    /**
     * Validate input is a valid IPv4 address (x.x.x.x format with valid octets 0-255)
     * @param {string|Object} [options] - Error message or options object
     * @param {string} [options.message] - Custom error message
     * @returns {Function} Validator function
     */
    ipAddress: (options = {}) => async (input) => {
        const message = typeof options === 'string' ? options : (options.message || 'Invalid IP address');
