...
class RedisStorage extends StorageInterface {
     ...
     * @param {Object} [config.pool] - Connection pool options (for Redis Cluster or Sentinel)
     * @param {number} [config.pool.maxConnections=10] - Maximum number of connections
     * @param {number} [config.pool.minConnections=2] - Minimum idle connections
     * @param {number} [config.pool.connectionTimeout=5000] - Connection timeout in ms
     * @param {number} [config.pool.idleTimeout=30000] - Idle connection timeout in ms
     */
    constructor(config = {}) {
        super();
        ...
        this.poolConfig = config.pool || null;
    }
    ...
    async _ensureConnected() {
        ...
        if (!this.client) {
            ...
            if (this.poolConfig) {
                ...
                if (this.poolConfig.connectionTimeout) {
                    clientOptions.socket.connectTimeout = this.poolConfig.connectionTimeout;
                }
                if (this.poolConfig.reconnectStrategy !== undefined) {
                    clientOptions.socket.reconnectStrategy = this.poolConfig.reconnectStrategy;
                }
            }
    ...
    async withTransaction(fn) {
        ...
        // Collect operations, then execute them in a MULTI/EXEC block
        const operations = [];
        const proxy = {
            setState: (sessionId, state, timeout) => {
                operations.push(
                    (multi) => multi.hSet(this._getKey(sessionId), 'state', state),
                    (multi) => multi.expire(this._getKey(sessionId), timeout)
                );
            },
            setData: (sessionId, data, timeout) => {
                operations.push(
                    (multi) => multi.hSet(this._getKey(sessionId), 'data', JSON.stringify(data)),
                    (multi) => multi.expire(this._getKey(sessionId), timeout)
                );
            },
            deleteSession: (sessionId) => {
                operations.push(
                    (multi) => multi.del(this._getKey(sessionId))
                );
            }
        };
