...
class I18n {
     ...
     * @param {number} [options.cacheSize=1000] - Maximum cache size for translations
     */
    constructor(options = {}) {
        ...
        this._cache = new LRUCache({ maxSize: options.cacheSize || 1000, ttl: 0 });
    ...
    t(key, options = {}) {
        ...
        // Build cache key from all options that affect the result
        const cacheKey = `${language}:${key}:${count !== undefined ? count : ''}:${JSON.stringify(params)}`;
    ...
    _interpolate(text, params) {
        ...
        // Skip regex if no placeholders present (performance optimization)
...
function createUSSDI18n(options = {}) {
    ...
    // Amharic translations (Ethiopia)
    i18n.addTranslations('am', {
        common: {
            welcome: 'እንኳን ደህና መጡ',
            goodbye: 'ደህና ሁኑ',
            thankYou: 'አመሰግናለሁ',
            error: 'ስህተት ተከስቷል',
            invalidInput: 'ልክ ያልሆነ ግብዓት',
        ...
        validation: {
            ...
            tooShort: 'ግብዓቱ በጣም አጭር ነው',
            tooLong: 'ግብዓቱ በጣም ረጅም ነው'
        }
    });

    // Arabic translations (Middle East)
    i18n.addTranslations('ar', {
        ...
        validation: {
            ...
            tooShort: 'الإدخال قصير جداً',
            tooLong: 'الإدخال طويل جداً'
        }
    });

    // Portuguese translations (Mozambique/Angola)
    i18n.addTranslations('pt', {
        ...
        validation: {
            ...
            tooShort: 'Entrada muito curta',
            tooLong: 'Entrada muito longa'
        }
    });

    // Hausa translations (Nigeria)
    i18n.addTranslations('ha', {
        common: {
            welcome: 'Barka da zuwa',
            goodbye: 'Sai an jima',
            thankYou: 'Na gode',
            error: 'An sami kuskure',
            invalidInput: 'Shigarwar ba daidai ba',
            tryAgain: 'Don Allah a sake gwadawa',
            back: 'Komawa',
            next: 'Na gaba',
            cancel: 'Sokewa',
            confirm: 'Tabbatarwa',
            yes: 'Eh',
            no: "A'a",
            select: 'Zaɓi wani zaɓi',
            enter: 'Shigar',
            loading: 'Don Allah a jira...',
            timeout: 'Lokaci ya ƙare',
            mainMenu: 'Babban Menu'
        },
        validation: {
            required: 'Ana buƙatar wannan filin',
            invalidPhone: 'Lambar wayar ba daidai ba',
            invalidEmail: 'Adireshin imel ba daidai ba',
            invalidAmount: 'Adadin ba daidai ba',
            tooShort: 'Shigarwar gajere ce sosai',
            tooLong: 'Shigarwar doguwa ce sosai'
        }
    });
