        ...
        const next = async () => {
            if (index < middlewares.length) {
                ...
                try {
                    ...
                    error.middlewareHook = hook;
                    error.middlewareIndex = currentIndex;
                    error.middlewareName = middleware.name || `anonymous-${currentIndex}`;
                    throw error;
                }
            }
        };
...
function createMetricsMiddleware(options = {}) {
    ...
    // Use circular buffer for O(1) operations instead of O(n) shift()
    const metrics = {
        totalRequests: 0,
        totalErrors: 0,
        requestsByState: {},
        _responseTimeBuffer: new Float64Array(bufferSize),
        _bufferIndex: 0,
        _bufferCount: 0,
        _responseTimeSum: 0
    };
    ...
    const middleware = async (context, next) => {
        ...
        try {
            ...
            if (metrics._bufferCount >= bufferSize) {
                metrics._responseTimeSum -= metrics._responseTimeBuffer[metrics._bufferIndex];
            } else {
                metrics._bufferCount++;
            }

            metrics._responseTimeBuffer[metrics._bufferIndex] = duration;
            metrics._responseTimeSum += duration;
            metrics._bufferIndex = (metrics._bufferIndex + 1) % bufferSize;
        }
    };
    ...
    const getMetrics = () => ({
        ...
        averageResponseTime: metrics._bufferCount > 0
            ? Math.round(metrics._responseTimeSum / metrics._bufferCount) + 'ms'
            : '0ms'
    });
    ...
    const resetMetrics = () => {
        ...
        metrics._responseTimeBuffer.fill(0);
        metrics._bufferIndex = 0;
        metrics._bufferCount = 0;
        metrics._responseTimeSum = 0;
    };
...
/**
 * Create a distributed rate limiting middleware using Redis
 *
 * Uses a sliding window counter algorithm for accurate rate limiting
 * that works correctly across multiple application instances.
 *
 * Requires a connected Redis client (from the 'redis' npm package).
 *
 * @param {Object} options - Rate limit options
 * @param {Object} options.redisClient - Connected Redis client instance
 * @param {number} [options.maxRequests=10] - Max requests per window
 * @param {number} [options.windowMs=60000] - Time window in milliseconds
 * @param {string} [options.keyPrefix='ussd:ratelimit:'] - Redis key prefix
 * @param {string} [options.message='Too many requests. Please try again later.'] - Error message
 * @returns {Object} Object containing middleware function and reset method
 *
 * @example
 ...
 */
function createDistributedRateLimitMiddleware(options = {}) {
    const {
        redisClient,
        maxRequests = 10,
        windowMs = 60000,
    ...
    if (!redisClient) {
        throw new Error('Distributed rate limiting requires a redisClient option');
    }

    const windowSeconds = Math.ceil(windowMs / 1000);

    const middleware = async (context, next) => {
        const key = `${keyPrefix}${context.sessionId}`;
        const now = Date.now();
        ...
        try {
            ...
            multi.expire(key, windowSeconds + 1); // Set TTL for cleanup
            ...
            // to prevent rate limiter outage from blocking all traffic
        }
