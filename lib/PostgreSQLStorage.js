...
class PostgreSQLStorage extends StorageInterface {
    ...
    async pushStateHistory(sessionId, state) {
        ...
        await this.pool.query(
            `UPDATE ${this.table} 
       SET state_history = array_append(state_history, $2), updated_at = NOW()
       WHERE session_id = $1`,
            [sessionId, state]
        );
    }
    ...
    async popStateHistory(sessionId) {
        ...
        const result = await this.pool.query(
            `UPDATE ${this.table} 
       SET state_history = state_history[1:array_length(state_history, 1)-1],
           updated_at = NOW()
       WHERE session_id = $1
       RETURNING state_history[array_length(state_history, 1)] as previous_state`,
            [sessionId]
        );
