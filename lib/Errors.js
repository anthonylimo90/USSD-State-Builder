 ...
 * Thrown when a handler returns an invalid response format
 */
class ResponseFormatError extends USSDError {
    /**
     * Create a Response Format Error
     * @param {string} response - The invalid response
     * @param {string} state - State that returned the invalid response
     */
    constructor(response, state) {
        const preview = response ? response.substring(0, 50) : '(empty)';
        super(
            `Invalid response format from state "${state}": must start with "CON " or "END ". Got: "${preview}"`,
            'RESPONSE_FORMAT_ERROR',
            { state, responsePreview: preview }
        );
